...
function createEvent(appId, emailId, groupId, title, responseEndTime, startTime, endTime, info, callback){
    var evnt = new EventsInfo({groupId:groupId, createdBy:emailId, title:title, info:info, responseEndTime:responseEndTime, startTime:startTime, endTime:endTime});
...
app.post("/api/app/newEvent", function(req, res){
    ...
    reqData["endTime"] = req.body.stopTime;
    reqData["info"] = req.body.information;
    console.log(reqData);
...
function updateEventStatus(appId, emailId, eventId, status, lastUpdatedAt, callback){
    //var EventStatusSchema = mongoose.Schema({eventId:String, studentId:String, lastUpdatedAt:String, Status:String});
    //var EventStatusSchema = mongoose.Schema({eventId:String, studentId:String, lastUpdatedAt:String, Status:String});
    console.log(appId, emailId, eventId, status, lastUpdatedAt);
    EventsStatus.findOne({eventId:eventId, studentId:emailId}, function(err, eventStatusObj){
        if (err) {
            console.log(err);
            callback(err,JSON.stringify({response:"error"}));
        }
            ...
            eventStatusObj = new EventsStatus({eventId:eventId, studentId:emailId, lastUpdatedAt:lastUpdatedAt, Status: status})
        }
...
app.post("/api/app/updateEventStatus", function(req, res){
    ...
    //reqData["admins"] = req.body.admins;
    //reqData["info"] = req.body.information;
    //appId, emailId, eventId, status, lastUpdatedAt
    console.log(reqData);
...
function sendSize(appId, emailId, eventId, callback){
    //var EventStatusSchema = mongoose.Schema({eventId:String, studentId:String, lastUpdatedAt:String, Status:String});
    //var EventStatusSchema = mongoose.Schema({eventId:String, studentId:String, lastUpdatedAt:String, Status:String});
    console.log(appId, emailId, eventId);
    EventsStatus.find({eventId:eventId}, function(err, eventStatusObj){
        var names = new Array();
        if (err) {
            console.log(err);
            callback(err,JSON.stringify({response:"error"}));
            ...
            eventStatusObj.forEach(function(e){
                names.push(e.studentId);
            });
            callback(null,JSON.stringify({"response":"success", "list":names}));
        }
        else{
            callback(null,JSON.stringify({"response":"success", "list":names}));
        }
...
app.post("/api/app/getSize", function(req, res){
    //console.log(req.body);
    //var UpdatesSchema = mongoose.Schema({groupId:String, postedBy:String, heading:String, text:String}); //groupId, emailId, text
    var reqData = new Array();
    reqData["appId"] = req.body.appId;
    ...
    //appId, emailId, eventId, status, lastUpdatedAt
    console.log(reqData);
    sendSize(reqData["appId"], reqData["emailId"], reqData["eventId"], function(err, result) {
        res.writeHead(200, {
            'Content-Type' : 'x-application/json'
        });
        res.end(result);
